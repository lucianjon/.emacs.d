;;; lj-modeline.el --- An awesome elisp package -*- lexical-binding: t; -*-

;; Copyright (C) 2018 Lucian Jones

;; Author: Lucian Jones <lucianm.jones@gmail.com>

;;; Commentary:
;;
;; Taken from https://github.com/jerrypnz/.emacs.d/blob/master/config/jp-base/jp-modeline.el
;;
;;; Code:
...
(defvar lj-modeline-active-window nil)

(defun lj-modeline-set-active-window (windows)
  (when (not (minibuffer-window-active-p (frame-selected-window)))
    (setq lj-modeline-active-window (selected-window))))

(add-function :before pre-redisplay-function #'lj-modeline-set-active-window)

(defun lj-modeline-active-p ()
  (eq lj-modeline-active-window (get-buffer-window)))

(defun lj-modeline-status ()
  (let* ((active (lj-modeline-active-p))
...
(setq-default
 ...
 '(
   ...
   (:eval (propertize (shorten-directory default-directory 10) 'face
                      (if (lj-modeline-active-p)
                          'mode-line-folder-face
                        'mode-line-folder-inactive-face)))
   (:eval (propertize (lj-buffer-filename) 'face
                      (if (lj-modeline-active-p)
